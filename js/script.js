...
function changeActiveParts(parts, value) {
    value ?
        parts.map(part => {
            ...
            document.getElementById(part.slice(0, -1) + 'Vol').classList.remove('off')
            document.getElementById(part.slice(0, -1) + 'Res').classList.remove('off')
        }) :
        parts.map(part => {
...
function chooseGlas(id) {
    ...
    document.getElementById('tropar').innerHTML = tropar[Number(id.replace('glas', '')) - 1]
    changeActiveParts(parts, false)
    switch (id) {
        case 'glas1': changeActiveParts(glas1Data[0], true); song = makeShortMelody(glas1Data[1], glas1Data[2]); break
        case 'glas2': changeActiveParts(glas2Data[0], true); song = makeShortMelody(glas2Data[1], glas2Data[2]); break
        case 'glas3': changeActiveParts(glas3Data[0], true); song = makeShortMelody(glas3Data[1], glas3Data[2]); break
        default: alert('Нет такой распевки'); song = ''; break
    }
    glas = id
    document.getElementById('player_short').src = song
}
...
function generateSong() {
    golosa = [
        document.getElementById('soprano1').checked,
        document.getElementById('soprano2').checked,
        document.getElementById('alt').checked,
        document.getElementById('tenor1').checked,
        document.getElementById('tenor2').checked,
        document.getElementById('bariton').checked,
        document.getElementById('bas').checked
    ]
    velocity = [
    ...
    switch (glas) {
        case 'glas1': song = makeMelody(glas1Data[3], glas1Data[4]); break
        case 'glas2': song = makeMelody(glas2Data[3], glas2Data[4]); break
        case 'glas3': song = makeMelody(glas3Data[3], glas3Data[4]); break
        default: alert('Нет такой распевки'); song = ''; return
    }
    document.getElementById('trackName').innerHTML = '"' + document.getElementById(glas).textContent + '" в тоне  "' + document.getElementById(toneKey).textContent + '" в темпе "' + document.getElementById(tempoKey).textContent + '"'
    ...
    melody.map((chord, i) => {
        chord.forEach((note, part) => {
            if (note !== 0 && part < chord.length - 1) {
                let partsWithSameNote = chord.map((n, j) => note === n ? j : '').filter(String)
                if (partsWithSameNote.length > 1) {
                    let volumesOfParts = partsWithSameNote.map(ind => volume[ind])
                    let partWithMaxVolume = partsWithSameNote[volumesOfParts.indexOf(Math.max.apply(null, volumesOfParts))]
                    partsWithSameNote.map(index => index !== partWithMaxVolume ? chord[index] = 0 : 0)
                }
            }
        })
        chord.map((note, part) => tracks[part].addNote(0, note, temp[part][i], 0, volume[part]))
    })
    console.log(melody)
